			...
			[
				{
					...
					component: (ctx) => ({
						button: new ButtonBuilder()
							.setLabel(
								!ctx.selectedPost?.title || !ctx.selectedPost.description
						...
						modal: new ModalBuilder().setTitle('Edit Information').addComponents([
				...
				},
				{
					...
					component: (ctx) => ({
						button: new ButtonBuilder()
							.setLabel(
								!Object.values(ctx.selectedPost!.paymentTypes).filter(Boolean).length
									? '* Edit payment'
									: 'Edit payment',
							)
							.setStyle(
						...
						modal: new ModalBuilder().setTitle('Edit Payment').addComponents([
							new ActionRowBuilder<TextInputBuilder>().addComponents([
								new TextInputBuilder()
									.setCustomId('robux')
									.setLabel('Robux')
					...
					callback: (ctx, interaction) => {
						...
						ctx.selectedPost!.paymentTypes = {
							Percentage:
								Number(percentage) && Number(percentage) <= 100 ? Number(percentage) : undefined,
			...
			[
				{
					...
					component: (ctx) =>
						new StringSelectMenuBuilder().setPlaceholder('* Select payment type').addOptions([
							{
								label: 'Upon completion',
								value: 'Upon completion',
								default: ctx.selectedPost?.paymentType === 'Upon completion',
							},
							{
								label: 'Per-task',
								value: 'Per-task',
								default: ctx.selectedPost?.paymentType === 'Per-task',
							},
							{
								label: 'Upfront',
								value: 'Upfront',
								default: ctx.selectedPost?.paymentType === 'Upfront',
							},
						]),
					callback: (ctx) => {
